...
namespace Kinematicula.Mathematics
{
    public struct Position3D : IEquatable<Position3D>
    {
        ...
        public bool EqualsTo(Position3D other, int precision)
        {
            var epsilon = Math.Pow(10.0, -precision);
            return (Math.Abs(Vector.X - other.Vector.X) < epsilon)
                    && (Math.Abs(Vector.Y - other.Vector.Y) < epsilon)
                    && (Math.Abs(Vector.Z - other.Vector.Z) < epsilon);
        }
