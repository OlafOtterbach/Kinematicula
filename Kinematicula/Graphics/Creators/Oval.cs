...
public static class Oval
{
        ...
        Color edgeColor)
    {
        ...
        var (topLoop1, topLoop2) = CreateTopBottomFace(segments1, segments2, radius1, radius2, length, depth, depth / 2.0, creator, true, topColor);
        var (bottomLoop1, bottomLoop2) = CreateTopBottomFace(segments1, segments2, radius1, radius2, length, depth, -depth / 2.0, creator, false, bottomColor);
        CreateSideFace(topLoop1, topLoop2, bottomLoop1, bottomLoop2, hasBorder1, hasBorder2, facetted1, facetted2, creator, sheatColor);
        var body = creator.CreateBody(edgeColor);
        return body;
    }

    private static (List<Position3D>, List<Position3D>) CreateTopBottomFace(int segments1, int segments2, double radius1, double radius2, double length, double depth, double z, GraphicsCreator creator, bool isTop, Color color)
    {
    ...
    private static void CreateSideFace(
        ...
        Color color)
    {
        creator.AddFace(hasBorder1, facetted1, color);
        ...
        creator.AddFace(hasBorder2, facetted2, color);
    ...
    {
        var creator = new GraphicsCreator();
        var topLoop = CreateTopBottomFace(segments, radius1, radius2, length, depth, depth, creator, true, topColor);
        var bottomLoop = CreateTopBottomFace(segments, radius1, radius2, length, depth, 0.0, creator, false, bottomColor);
        CreateSideFace(topLoop, bottomLoop, creator, sheatColor);
